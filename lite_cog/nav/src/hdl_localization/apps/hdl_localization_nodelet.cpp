...
public:
  ...
  void onInit() override {
    ...
    leg_odom2_sub = nh.subscribe("/leg_odom2", 10, &HdlLocalizationNodelet::leg_odom2_callback, this);
...
private:
  ...
  void initialize_params() {
    imu_timeout_ms = private_nh.param<int>("imu_timeout_ms", 10);
    lidar_timeout_ms = private_nh.param<int>("lidar_timeout_ms", 200);
    ...
    // read NDT quality gate params
    require_ndt_convergence = private_nh.param<bool>("require_ndt_convergence", true);
    max_correction_dist = private_nh.param<double>("max_correction_dist", 2.0);
    max_correction_angle = private_nh.param<double>("max_correction_angle", 0.5);
...
private:
  ...
  void imu_callback(const sensor_msgs::ImuConstPtr& imu_msg) {
    auto t_diff_ms = (ros::Time::now() - imu_msg->header.stamp).toSec() * 1000;
    if(t_diff_ms < imu_timeout_ms){
      std::lock_guard<std::mutex> lock(imu_data_mutex);
      imu_data.push_back(imu_msg);
    }
    else{
      ROS_WARN("Disgarding a timeout imu msg, t_diss: %lf ms", t_diff_ms);
    }
  }
  ...
  void points_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    auto t_diff_ms = (ros::Time::now() - points_msg->header.stamp).toSec() * 1000;
    if(t_diff_ms > lidar_timeout_ms){
      ROS_WARN("Disgarding a timeout lidar msg, t_diss: %lf ms", t_diff_ms);
      return;
    }
    ...
    // throttled warning when NDT correction is skipped (quality gate)
    if (pose_estimator->last_correction_skipped()) {
      ROS_WARN_THROTTLE(2.0, "NDT correction skipped — localization degraded, relying on prediction");
    }
  ...
  void leg_odom2_callback(const nav_msgs::OdometryConstPtr& msg){
    leg_odom2_msg = msg;
  }
  ...
  void publish_odometry(const ros::Time& stamp, const Eigen::Matrix4f& pose) {
    ...
    if (leg_odom2_msg) {
      odom.twist = leg_odom2_msg->twist;
    }
...
private:
  ...
  nav_msgs::OdometryConstPtr leg_odom2_msg;
