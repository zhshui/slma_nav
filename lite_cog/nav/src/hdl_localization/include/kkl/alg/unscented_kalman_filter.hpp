...
private:
  ...
  void ensurePositiveFinite(MatrixXt& cov) {
    ...
    // enforce symmetry (numerical drift can break SelfAdjointEigenSolver)
    cov = 0.5 * (cov + cov.transpose());

    Eigen::SelfAdjointEigenSolver<MatrixXt> solver(cov);
    if (solver.info() != Eigen::Success) {
      // eigen-decomposition failed — reset to identity
      cov = MatrixXt::Identity(cov.rows(), cov.cols()) * eps;
      return;
    }

    MatrixXt D = solver.eigenvalues().asDiagonal();
    MatrixXt V = solver.eigenvectors();
