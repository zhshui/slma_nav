...
public:
  ...
  virtual ~HdlGraphSlamNodelet() {
    if(auto_save_on_exit && !keyframes.empty()) {
      ...
      std::string dest = auto_save_path;
      boost::filesystem::path p(dest);
      std::string stem = p.stem().string();
      std::string ext = p.extension().string();
      int idx = 1;
      while(boost::filesystem::exists(dest)) {
        dest = (p.parent_path() / (stem + "_" + std::to_string(idx) + ext)).string();
        idx++;
      }

      NODELET_INFO("Auto-saving PCD to %s", dest.c_str());
      pcl::io::savePCDFileBinary(dest, *cloud);
    }
  }
...
private:
  ...
  bool save_map_service(hdl_graph_slam::SaveMapRequest& req, hdl_graph_slam::SaveMapResponse& res) {
    ...
    // auto-rename with incrementing number to avoid overwriting
    ...
    if(zero_utm) {
      std::ofstream ofs(dest + ".utm");
      ofs << boost::format("%.6f %.6f %.6f") % zero_utm->x() % zero_utm->y() % zero_utm->z() << std::endl;
    }

    int ret = pcl::io::savePCDFileBinary(dest, *cloud);
    res.success = ret == 0;
