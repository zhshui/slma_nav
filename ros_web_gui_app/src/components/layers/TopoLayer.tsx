...
export class TopoLayer extends BaseLayer {
  ...
  private createLabel(text: string): THREE.Group {
    ...
    // 根据文字长度动态调整 canvas 尺寸，避免大量透明区域产生白边
    const textWidth = context.measureText(text).width;
    canvas.width = Math.max(64, Math.ceil(textWidth + 24));
    canvas.height = Math.ceil(fontSize * 1.6);
    // 重新设置 font（canvas 尺寸改变会重置 context 状态）
    context.font = `bold ${fontSize}px Arial`;
    ...
    // scale 根据实际 canvas 比例调整，保持文字大小合适
    const aspect = canvas.width / canvas.height;
    sprite.scale.set(aspect * 0.18, 0.18, 1);
